...
quintiles_data <- read.csv("Stuart2transposedSimple.csv", header = TRUE)
...
#### so need to get returns for the FOLLOWING MONTH, so create a new date on month later (port_calc_date)

quintiles_data$port_calc_date<-  as.Date(as.mondate(quintiles_data$Date) + 1)


############ trim this dataset, quintiles_data, to line up with the fama french dataset

### right now Fama frenchdata only goes to september 2017, so trim portfolio
...
write.csv(combined_dataframe, file = "combined_DF_simple.csv")
